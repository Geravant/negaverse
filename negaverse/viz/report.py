...
def _interactive_panel(out_dir: Path) -> str:
    ...
    head = ('<section class="panel"><h2>Interactive 3D map — drag to rotate, scroll to zoom, '
            ...
            '&bull; <b>x — looks like a real interaction:</b> how much the pair resembles real '
            'interactions. By default this is the <i>shape</i> of the known network — two proteins '
            'score high when linked through short chains of shared partners (friend-of-a-friend '
            'patterns interacting proteins tend to show). On graphs too sparse for that (e.g. '
            'DRYAD) the x-axis instead reads a protein-<i>sequence</i> (ESM2) model trained to '
            'tell real interactors from known non-interactors; the axis label says which lens is '
            'active. High = it looks like it <i>should</i> interact.<br>'

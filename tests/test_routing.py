...
def test_contested_prioritises_disagreement_within_cap():
    ...
    # _contested returns the full priority-sorted tail (caller applies the cap);
    # the disagreement pair must sort first so a cap of 1 would keep it.
    picked = _contested(pool, pct=0.5, disagree_keys={("a", "b")})
    assert (picked[0].u, picked[0].v) == ("a", "b")           # disagreement sorts first
